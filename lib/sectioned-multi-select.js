...
export class SectionedMultiSelect extends PureComponent {
  static propTypes = {
    ...
    filterMatchKeys: PropTypes.array,
  ...
  _filterItems = (searchTerm) => {
    const { items, subKey, uniqueKey, displayKey, filterItems,filterMatchKeys } = this.props
    ...
    items &&
      items.forEach((item) => {
        ...
        this.getProp(item, filterMatchKeys).forEach(key => {
          // what if the object item's key is not a string?
          matchTerm = ` ${matchTerm + item[key]}`
        })

        if (regex.test(matchTerm ?? this.getProp(item, displayKey))) {
